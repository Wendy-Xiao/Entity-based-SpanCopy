...
class spanCopySummarizer(pl.LightningModule):
    ...
    def compute_rouge_batch(
        self, input_ids, output_ids, input_ent_mapping_batch, labels, fid,use_gt_gr=False
    ):
        # scorer = load_metric(
        ...
        if use_gt_gr:
            gr_labels = torch.zeros((input_ent_mapping_batch.shape[0],input_ent_mapping_batch.shape[2]), device=input_ids.device)
